...
GROUP_ID=$(aws ec2 get-security-groups-for-vpc --region ${REGION} --vpc-id ${VPC_ID} --filter Name="group-name",Values="${SG_NAME}" --query "SecurityGroupForVpcs[].GroupId" --output text)
